...
다음 이미지는 Animal에서 생성된 .asu 파일을 실행한 결과이다. g1, g2 각각 초기 그래프, 시작 노드와 후보 노드 표시, 중간 DFS 진행 단계, DFS 완료 화면을 포함한다.
...
파일 경로: src/dfs_animal_generator.py
...
- 입력 그래프 기반 DFS 수행을 완료했다.
- Python 프로그램으로 AnimalScript 자동 생성을 완료했다.
- g1, g2 Animal 실행 결과를 확보했다.
- 노드 색상과 굵은 DFS 트리 간선을 사용해 DFS 과정을 시각화했다.
- LMS 제출 시 이 문서를 Word/HWP 형식 하나로 제출한다.